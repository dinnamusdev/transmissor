...
namespace SincronizadorDeTabelas
{
    class EnviarMetodosBasicos
    {
        ...
        /*
         ...
         Function GerarArquivos(cLocal As String, cDataUltimoSincronismo As String, nCodigoLoja As Integer, Optional nIdEnvio As Long, Optional dData As Date, Optional cHora As String, Optional bCargaTotal As Boolean) As Collection
On Error GoTo ksdkasjk
Dim clArquivos As New Collection
Dim cConteudoArquivo As String
Dim cNomeArquivo As String
Dim rstTmp As New ADODB.Recordset

Dim cNomeTabela As String
Dim nRetorno As Integer


Set rstTmp = Sincronismo_ListaTabelaSincronismo(nCodigoLoja)
With rstTmp
     If .RecordCount > 0 Then
        While Not .EOF
                    
                    cNomeTabela = .Fields("NomeTabela").value
                    
                    Sincronismo_MensagemAoOperador_Envio "Processando Tabela " & cNomeTabela
                    
                    cNomeArquivo = Format(.Fields("Ordem").value, "0000") & "-" & cNomeTabela & "#" & .Fields("ChavePrimaria").value & "#" & Format(dData, "ddmmyy") & "#" & Format(cHora, "hhmmss") & ".din"
                    
                    nRetorno = GerarArquivoTabela(CurDir & "\" & cNomeArquivo, cNomeTabela, cDataUltimoSincronismo, bCargaTotal)
                    If nRetorno = 0 Then
                        Set GerarArquivo = New Collection
                        Exit Function
                    Else
                        If nRetorno = 1 Then
                           clArquivos.Add cNomeArquivo
                        End If
                    End If
                    Sincronismo_MensagemAoOperador_Envio "OK" & vbCrLf
                    
                   .MoveNext
        Wend
        
        If clArquivos.Count > 0 Then
            If nIdEnvio = 0 Then
                Sincronismo_RegistrarEnvio dData, cHora
            End If
            Dim cNomeArquivoRegistroTransf As String
                    
            cNomeArquivoRegistroTransf = Sincronismo_GerarArquivoRegistroTransferencia(nIdEnvio, dData, cHora)
             
            If Len(Trim(cNomeArquivoRegistroTransf)) > 0 Then
               clArquivos.Add cNomeArquivoRegistroTransf
            Else
               Set GerarArquivos = New Collection
            End If
        End If
    Else
        MsgBox "Não foram localizados os registros da tabelas a serem transmitidas. Procedimento Cancelado", vbExclamation
        Set GerarArquivo = New Collection
    End If
End With


Set GerarArquivos = clArquivos
...
End Function
        ...
        public static DataSet Sincronismo_ListaTabelaSincronismo(Int16 nCodigoLoja) {
            ...
            try
            {
                DataSet dsTmp = DAO.Pesquisar("select isnull(codigolojamatriz,0) codigolojamatriz from lojas where codigo=" + nCodigoLoja);
                if (dsTmp!=null)
                {
